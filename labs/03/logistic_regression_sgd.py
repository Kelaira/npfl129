    ...
    for iteration in range(args.iterations):
        ...
        for batch in range(n_batches):
            ...
            for sample in range(args.batch_size):
                ...
                prediction = np.dot(np.transpose(x), weights)

                if y == 0:
                    der_prob = np.exp(prediction) / (1 + np.exp(prediction))
                else:
                    der_prob = -1 / (1 + np.exp(prediction))

                gradient_sum += np.dot(der_prob, x)

            grad = gradient_sum / args.batch_size
        ...
        def calculate_accuracy_loss (data, target):
            loss = 0
            n_correct = 0
            total = data.shape[0]
            for i in range(total):
                ...
                prob = (sigmoid if y == 1 else 1 - sigmoid)
                if prob > 0.5:
                    n_correct += 1
        ...
        train_accuracy, train_loss = calculate_accuracy_loss(train_data, train_target)
        test_accuracy, test_loss = calculate_accuracy_loss(test_data, test_target)
